...
import { DOMAIN_DATA_VERSION, domainDataV1Schema } from "@/src/typing/data";
import {
  CommandRegistry,
  requestSchemas,
...
type ResponseHandler<K extends keyof CommandRegistry> = (
  ...
  // this is based on the actual interface for
...
const _responseHandlers: ResponseRegistry = {
  ...
  load_data: (message, baseSendResponse) => {
    ...
    const data = storage.getStoredURLData(url);
    if (data === undefined) {
  ...
  upgrade_domain_data: (message, baseSendResponse) => {
    ...
    if (!parsed.success) {
      sendResponse<"upgrade_domain_data">(baseSendResponse, {
        success: false,
        message: `Validation error during upgrade:\n${z.prettifyError(parsed.error)}`,
      });
      return;
    }

    const upgraded = storage.upgradeData(parsed.data);
    if (upgraded.success) {
...
type GenericResponseRegistry = {
  [K in keyof CommandRegistry]: (
    // having to circumvent typescript a little due to it not working
    // too well with some stuff, see below at responseHandlers
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    message: any,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    baseSendResponse: (response?: any) => void,
